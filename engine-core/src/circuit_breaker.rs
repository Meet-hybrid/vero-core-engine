...
use soroban_sdk::{contracterror, panic_with_error, symbol_short, vec, Address, Env, Symbol, Vec, BytesN, Map};
...
pub fn trip(env: &Env, guardian: &Address) {
    ...
    payload.set(symbol_short!("guard"), guardian.clone().into_val(env));
